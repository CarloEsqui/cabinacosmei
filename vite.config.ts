...
export default defineConfig(({ command }) => {
  // El watcher de main/preload SOLO debe existir en `vite` (dev server). En `vite build`
  // (producción/CI), cualquier `build.watch` definido hace que Rollup/Rolldown entre en modo
  // watch y el proceso nunca termina — colgaba `electron-builder` indefinidamente en CI.
  const watchMainPreload =
    command === "serve"
      ? {
