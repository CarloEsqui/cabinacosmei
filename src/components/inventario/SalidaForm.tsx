...
/**
 * Formulario de salida de inventario, reutilizable dentro de un modal. Las salidas siempre son en
 * piezas enteras (vender/mermar descuenta piezas, no fracciones de contenido).
 * `onRegistrado` se llama tras una salida exitosa (el modal contenedor la usa para cerrarse).
 */
export function SalidaForm({ onRegistrado }: { onRegistrado?: () => void }) {
  ...
  const registrar = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Salida registrada. El inventario ya se actualizó.");
  ...
  return (
    ...
    >
          ...
          {lotesDisponibles
            ...
            .map((l) => (
              <option key={l.id} value={l.id}>
                {(l.numeroLote || l.id.slice(0, 8))} — disponible{" "}
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-500">
          Cantidad ({productoSel?.presentacion ? `${productoSel.presentacion.toLowerCase()}s` : "piezas"}) *
        </label>
        <NumberInput
          min={0}
          step="0.01"
